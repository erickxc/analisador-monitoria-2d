...
| Chave | Rótulo | Função | O que é |
|---|---|---|---|
| `alto_giro` | Alto Giro | `status_alto_giro()` | **Sem granularidade** (sempre mês-calendário, compara só os 2 últimos meses). Considera só clientes do **Grupo 1** (classificação agregada por receita total, mesmo corte de `classificar_clientes_agregado` usado em Poder de Compra/Sem Venda/prévia de Configurações) — receita, status e clientes destaque/em queda refletem só a carteira principal; um produto sem nenhuma compra de cliente Grupo 1 nos 2 últimos meses não aparece. Uma linha por produto **presente na base recebida** — a função não faz corte de produto nenhum, só reflete o que já chegou filtrado (normalmente a lista "considerado" de alto giro); desmarcar um produto em Configurações some com ele daqui automaticamente, sem lógica de "substituição" nenhuma. Colunas: `Receita Atual` (mês mais recente); `Status` (Em alta/Em queda/Estável) e `% de Variação` vs. o mês anterior; `Cliente Destaque` (quem mais comprou esse produto no mês, EXCETO quem estiver em `Cliente em Queda` nesse mesmo produto — um cliente cuja compra caiu não é "destaque", mesmo sendo o maior comprador do mês; o posto passa para o próximo) com `% Variação do Cliente Destaque` (a compra DELE nesse produto, mês atual vs. anterior — não confundir com o `% de Variação` do produto como um todo); `Cliente em Queda` (quem mais reduziu a compra desse produto vs. o mês anterior, EXIGINDO que ainda tenha comprado algo no mês atual — um cliente que foi a zero não conta aqui, isso é assunto de Erosão/Sem Venda; "—" se ninguém se encaixar) com `% Variação do Cliente em Queda` (idem, sempre negativa). Ambas as variações ficam em branco quando o cliente não comprou nada no mês anterior (sem base de comparação). Cliente-balcão (venda de balcão/consumidor final) não entra na escolha desses dois clientes quando o checkbox de balcão estiver marcado — senão domina como destaque/queda em quase todo produto, sem ser um cliente real endereçável. Ordenado por `Receita Atual` descendente. |
