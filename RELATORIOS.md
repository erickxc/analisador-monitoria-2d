...
| `abc` | Faturamento e Segmentação de Clientes (ABC) | `classificar_abc` → `classificar_faixas` | Por período, ordena clientes pela receita **acumulada cronologicamente até aquele período** (soma corrida desde o primeiro período em que o cliente aparece na base carregada — não a receita isolada daquele mês) e divide em faixas por % acumulado dessa curva (`Grupo 1/2/3/Demais`, cortes padrão 30/50/60%, mais faixa **"Balcão"** própria se o checkbox de balcão estiver marcado — com `Percentual_Individual` real, `Percentual_Acumulado` = NaN, nunca misturada com "Grupo 1"). Mesmo conceito de "Grupo" usado em `classificar_clientes_agregado` (Poder de Compra/Alto Giro), só que recalculado a cada período em vez de fixo pra todo o histórico — um mês fraco isolado não derruba a faixa de quem já tem histórico grande (a cicatriz em R$ absolutos não desaparece, mas fica proporcionalmente menor conforme mais meses se acumulam; como a base carregada normalmente já é uma janela de ~11 meses, meses antigos saem de cena naturalmente quando a base é atualizada — não é um "acumulado desde o início dos tempos" sem fim). Nova coluna `Receita_Acumulada_Total` guarda esse valor; `Receita` continua sendo a receita isolada daquele mês (usada por `Renuncia*` e pra mostrar atividade real). Mostra só os **5 clientes de maior `Receita_Acumulada_Total` por (período, faixa)** — `top_clientes_por_grupo`, ajustável só via código — por isso tende a repetir os mesmos 5 nomes entre períodos vizinhos, diferente de antes (quando mudava a cada mês pelos maiores vendedores daquele mês isolado). Traz `Renuncia*` (queda de receita vs. período anterior); **não** traz mais frequência (removida — não fazia sentido junto de um corte "top 5"). |
| `migracao_abc` | Migração de Grupo (inclui resumo e score por cliente) | `migracao_abc()` + `resumo_migracao()` + `pontuacao_migracao_clientes()` | Marcar esta chave gera **3 abas**: (1) `Migracao_ABC` — quem subiu/desceu de faixa entre períodos consecutivos, com causa quando uma heurística bate com folga (produto abandonado ≥70% da receita, frequência caiu pela metade+, ticket médio caiu 40%+, ou produto novo ≥50% da receita atual — célula fica **em branco** se nada bater, sem "estimativa" genérica); (2) `Migracao_Resumo` — `Qtd_Subiu`/`Qtd_Desceu` por transição de período, mais `Margem` (`Qtd_Subiu - Qtd_Desceu` — positiva quando mais clientes subiram do que desceram na transição); (3) `Migracao_Score_Clientes` — score acumulado por cliente em todo o histórico (+2 por subida, -3 por queda), `Meses_No_Grupo_Atual` — sequência de períodos mais recentes e consecutivos (sem buraco no calendário) na MESMA faixa em que o cliente está agora, não uma média/proporção do histórico inteiro (um cliente que passou o ano oscilando entre faixas e só emplacou na atual nos 2 últimos meses mostra "2", não uma média arrastando o passado; campo antigo `Percentual_Permanencia` fazia exatamente essa mistura e foi substituído por pedido do usuário, que validou o cálculo contra dados reais) — "transições" (usado só pra `Qtd_Subiu`/`Qtd_Desceu`/`Score`) conta só **pares de períodos consecutivos** em que o cliente aparece nos dois lados (mesma regra de `migracao_abc`), não simplesmente "nº de períodos em que aparece menos 1" (isso superestimaria as oportunidades pra clientes com histórico "com buracos" — ex.: compra em ago/25, some, volta só em dez/25) — e `Grupo` (faixa ABC do cliente no período mais recente em que aparece na base, não a faixa "no auge"). A classificação ABC usada aqui é sempre a **completa** (sem o corte de top-5 do relatório "ABC"), senão a migração só enxergaria 5 clientes por grupo. A faixa de cada cliente em cada período vem de `classificar_faixas` (ver relatório "abc" acima) — ranqueada pela receita acumulada até aquele período, não pelo mês isolado; um cliente só migra de fato quando essa posição muda de verdade, não por ruído de um mês fraco/forte pontual. |
...
| Chave | Rótulo | Função | O que é |
|---|---|---|---|
| `alto_giro` | Alto Giro | `status_alto_giro()` | **Sem granularidade** (sempre mês-calendário, compara só os 2 últimos meses). `Receita Atual`/`Status`/`% de Variação` consideram só clientes do **Grupo 1** (classificação agregada por receita total, mesmo corte de `classificar_clientes_agregado` usado em Poder de Compra/Sem Venda/prévia de Configurações) — um produto sem nenhuma compra de cliente Grupo 1 nos 2 últimos meses não aparece. Já `Cliente Destaque`/`Cliente em Queda` são outra métrica — não é maior faturamento, é quem mais **cresceu** (Destaque) e quem mais **caiu** (Em Queda) em % de variação da própria compra desse produto — por isso consideram clientes de **todos os grupos** (o corte de Grupo 1 vale só pra receita/status do produto, não pra essa comparação individual), só que **priorizando Grupo 1 > Grupo 2 > Grupo 3 > demais**: só usa um cliente de faixa "pior" quando não há nenhum candidato válido nas faixas melhores para aquele produto; dentro da mesma faixa, desempate por maior % de variação. Exigem compra > 0 nos DOIS períodos (desconsidera quem apareceu do zero ou sumiu de vez — isso é assunto de Erosão/Sem Venda) e uma variação no sentido certo (Destaque só com % positivo, Em Queda só com % negativo — "—" se ninguém se encaixar); nunca o mesmo cliente nos dois papéis no mesmo produto. Uma linha por produto **presente na base recebida** — a função não faz corte de produto nenhum, só reflete o que já chegou filtrado (normalmente a lista "considerado" de alto giro); desmarcar um produto em Configurações some com ele daqui automaticamente, sem lógica de "substituição" nenhuma. Cliente-balcão (venda de balcão/consumidor final) não entra na escolha de Destaque/Em Queda quando o checkbox de balcão estiver marcado — senão domina como destaque/queda em quase todo produto, sem ser um cliente real endereçável. Ordenado por `Receita Atual` descendente. |
| `alertas_queda` | Alertas de Queda Consecutiva | `tendencia_produtos()` (segundo retorno) | Produtos com N períodos seguidos de queda **terminando no período mais recente** (queda atual, não um histórico antigo já recuperado), na granularidade escolhida (N = "Períodos seguidos em queda p/ alerta", padrão 2). Colunas: `Períodos Consecutivos em Queda`; `Período Anterior à Queda`/`Receita Precedente à Queda`/`Qtd Precedente à Queda` (rótulo legível, ex.: `fev/26`, e receita/quantidade no período-base antes da primeira queda da sequência); `Receita Atual`/`Qtd Atual` (fim da sequência — sempre o período mais recente; o rótulo do período em si não é uma coluna porque seria o mesmo valor em toda linha); `Queda em R$` (soma acumulada da perda em CADA período da sequência frente ao período-base — não só a diferença entre o primeiro e o último; uma queda que persiste por vários períodos pesa mais que uma queda pontual do mesmo tamanho no último período); `% Média de Queda` (média das variações percentuais dentro da própria sequência, magnitude positiva). Nada disso são os extremos de todo o histórico do produto, só da sequência detectada. Ordenado por `Queda em R$` descendente — ainda prioriza impacto financeiro acumulado, não duração pura: um produto de alto volume caindo poucos períodos pode acumular mais perda que um de baixo volume caindo há muito mais tempo, e isso é esperado. Campo "Queda mínima em R$ p/ alerta" (padrão 3000, ajustável — 0 = sem piso), abaixo do checkbox deste relatório, filtra fora quedas pequenas antes de aplicar "Produtos a exibir". Meses/produtos com receita líquida negativa no período (devoluções/estornos que superam as vendas) não entram no cálculo de variação — o percentual perde sentido de magnitude quando a base de comparação é negativa. |
...
| `sem_venda` | Sem Venda | `sem_venda()` | **Sem granularidade** (sempre mensal, mesmo padrão de `erosao_clientes`/`poder_compra_clientes`). Reaproveita o mesmo motor de `erosao_clientes_geral` (`_erosao_generico`, pico vs. último mês), mas com corte padrão de 90% (não 50%) e **sem piso de R$** de propósito — pega clientes de baixo volume que Erosão descarta pelo piso financeiro padrão (R$3000), mas que mesmo assim pararam de comprar de verdade. Diferente de todos os outros relatórios de erosão (que só mostram pico x atual), a tabela aqui é **larga**: `Cliente`, `Grupo` (faixa ABC pela receita total), `Receita no Pico`, e uma coluna de receita por **cada mês disponível na base** (rótulo legível, ex. `ago/25`) — dá pra ver a trajetória completa (comprava bem, parou) de uma vez, não só dois pontos. Ordenado por `Receita no Pico` descendente. Filtro próprio "Redução mínima p/ Sem Venda (%)" (padrão 90, sem piso de R$ compartilhado com Erosão). Colunas de mês são dinâmicas (dependem do intervalo de datas da base carregada) — `COLUNAS_MOEDA_POR_ANALISE` não lista nomes fixos pra essa chave; `_colunas_moeda_efetivas()` em `app.py` resolve a formatação de moeda em tempo de exportação a partir do próprio DataFrame (tudo exceto `Cliente`/`Grupo`). |
