...
| `alertas_queda` | Alertas de Queda Consecutiva | idem (segundo retorno) | Produtos com N períodos seguidos de queda **na granularidade escolhida** (N = "Períodos seguidos em queda p/ alerta", padrão 2). Mesmo campo "Produtos a exibir" do item acima, aplicado por `Periodos_Consecutivos_Em_Queda` descendente. |
