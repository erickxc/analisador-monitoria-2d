...
| `alertas_queda` | Alertas de Queda Consecutiva | idem (segundo retorno) | Produtos com N períodos seguidos de queda **terminando no período mais recente** (queda atual, não um histórico antigo já recuperado), na granularidade escolhida (N = "Períodos seguidos em queda p/ alerta", padrão 2). Colunas: `Períodos Consecutivos em Queda`; `Período Anterior à Queda`/`Receita Precedente à Queda` (rótulo legível, ex.: `fev/26`, e receita no período-base antes da primeira queda da sequência); `Período Atual`/`Receita Atual` (fim da sequência — sempre o período mais recente); `% Média de Queda` (média das variações percentuais dentro da própria sequência, magnitude positiva). Nada disso são os extremos de todo o histórico do produto, só da sequência detectada. Mesmo campo "Produtos a exibir" do item acima, aplicado por `Períodos Consecutivos em Queda` descendente. Meses/produtos com receita líquida negativa no período (devoluções/estornos que superam as vendas) não entram no cálculo de variação — o percentual perde sentido de magnitude quando a base de comparação é negativa. |
| `erosao_clientes` | Erosão de Clientes por Produto | `erosao_clientes_por_produto()` | Por produto+cliente, compara só a transição de período **mais recente** (não o histórico inteiro) e lista quem reduziu ou zerou a compra. Filtro "Redução mínima p/ erosão" na tela (padrão 50%, ajustável) — só entra quem caiu pelo menos esse percentual. Colunas com `Cliente` antes de `descricao`. Se "Alertas de Queda" foi calculado, restrita aos produtos já sinalizados lá (senão roda para todos). Receita do período atual negativa (devolução/estorno que superou a venda) fica de fora — não é uma redução de compra no sentido normal, e o percentual passaria de 100%. |
